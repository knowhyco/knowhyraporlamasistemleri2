-- 17_Sistemin_Genel_ozeti
-- Parametreler:
-- {TABLE_NAME} - Table Name (örn. customer_denizmuzesi)

WITH user_counts AS (
    SELECT COUNT(DISTINCT session_id) AS total_sessions FROM {TABLE_NAME} --bu değişken adminin yazacağı tabloya göre değişmeli
),
message_counts AS (
    SELECT 
    ...
    FROM {TABLE_NAME} --bu değişken adminin yazacağı tabloya göre değişmeli
),
context_usage AS (
    SELECT 
